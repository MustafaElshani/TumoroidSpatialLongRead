...
# Step 2b: Barcode validation - verify alignment between Visium and ONT transcript data
# -------------------------------------------------------------------------------------
visium_barcodes <- spatlocs$cell_ID
ont_trans_barcodes <- colnames(SpatialTrans_DMSO_ONTtrans)
matched_barcodes <- intersect(visium_barcodes, ont_trans_barcodes)

cat("=== Barcode Validation (Transcript Level) ===\n")
cat("Visium spatial barcodes:", length(visium_barcodes), "\n")
cat("ONT transcript barcodes:", length(ont_trans_barcodes), "\n")
cat("Matched barcodes:", length(matched_barcodes), "\n")
cat("Visium-only barcodes (dropped):", length(setdiff(visium_barcodes, ont_trans_barcodes)), "\n")
cat("ONT-only barcodes (dropped):", length(setdiff(ont_trans_barcodes, visium_barcodes)), "\n")

# Step 3: Filter to matched barcodes only
# ---------------------------------------
filtered_spatlocs <- spatlocs[spatlocs$cell_ID %in% matched_barcodes, ]
SpatialTrans_DMSO_ONTtrans <- SpatialTrans_DMSO_ONTtrans[, matched_barcodes]

# Verify barcode order
stopifnot("Barcode order mismatch!" = all(filtered_spatlocs$cell_ID == colnames(SpatialTrans_DMSO_ONTtrans)))
...
SpatialTrans_DMSO_ONTtrans@cell_metadata$cell$rna@metaDT <- 
  SpatialTrans_DMSO@cell_metadata$cell$rna@metaDT[
    SpatialTrans_DMSO@cell_metadata$cell$rna@metaDT$cell_ID %in% matched_barcodes, ]
...
# Step 5: Quality Control for Transcript-level Data

## QC Strategy Note
# Comprehensive QC (%Mito, %Ribo) is performed at the gene-expression level in 
# GiottoSuite_ONTgene_DMSO.Rmd and GiottoSuite_ONTgene_30uM77738.Rmd.
# Spots passing gene-level QC (including %Mito and %Ribo assessment) are used here.
# For transcript-level analysis, we compute: total transcripts, detected isoforms,
# and transcript complexity metrics. Differential transcript usage is analyzed 
# using DRIMSeq (see GiottoSuite_DRIMseq.Rmd).
...
# Transcript-level QC metrics
trans_meta <- getCellMetadata(SpatialTrans_DMSO_ONTtrans_statistics, output = "data.table")

cat("=== Transcript-level QC Summary ===\n")
cat("Total spots:", nrow(trans_meta), "\n")
cat("\nTranscripts per spot (nr_feats):\n")
print(summary(trans_meta$nr_feats))
cat("\nTotal transcript counts per spot:\n")
print(summary(trans_meta$total_expr))

# Calculate transcript complexity (transcripts per unique gene equivalent)
# This is a proxy for isoform diversity
trans_expr <- getExpression(SpatialTrans_DMSO_ONTtrans, values = "raw", output = "matrix")
transcripts_detected <- colSums(trans_expr > 0)
cat("\nDistinct transcripts detected per spot:\n")
print(summary(transcripts_detected))
...
SpatPlot2_QC_nr_feats <- spatPlot2D(
  ...
  cell_color = "nr_feats",           # Color based on the number of features (transcripts) per spot
  point_size = 3,
  show_image = TRUE,                 # Show the underlying image
...
# Visualize the total expression per spot
