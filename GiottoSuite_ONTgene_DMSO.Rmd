...
# Step 3: Create Giotto Object and Load Visium Data
...
# Step 4: Replace with ONT Gene Expression Data

```{r step4_replace_ont_expression}
# Step 4.1: Load ONT gene expression data
# --------------------------------------
...
# Step 4.2: Get spatial locations from the original Giotto object
# ----------------------------------------------------------------
spatlocs <- getSpatialLocations(gobject = SpatialTrans_DMSO, 
                                output = "data.table")

# Step 4.3: Barcode validation - verify alignment between Visium and ONT data
# --------------------------------------------------------------------------
visium_barcodes <- spatlocs$cell_ID
ont_barcodes <- colnames(SpatialTrans_DMSO_ONTgene)
matched_barcodes <- intersect(visium_barcodes, ont_barcodes)

cat("=== Barcode Validation ===\n")
cat("Visium spatial barcodes:", length(visium_barcodes), "\n")
cat("ONT expression barcodes:", length(ont_barcodes), "\n")
cat("Matched barcodes:", length(matched_barcodes), "\n")
cat("Visium-only barcodes (dropped):", length(setdiff(visium_barcodes, ont_barcodes)), "\n")
cat("ONT-only barcodes (dropped):", length(setdiff(ont_barcodes, visium_barcodes)), "\n")

# Filter to matched barcodes only
spatlocs_filtered <- spatlocs[cell_ID %in% matched_barcodes]
SpatialTrans_DMSO_ONTgene <- SpatialTrans_DMSO_ONTgene[, matched_barcodes]

# Verify barcode order matches
stopifnot("Barcode order mismatch!" = all(spatlocs_filtered$cell_ID == colnames(SpatialTrans_DMSO_ONTgene)))
cat("Barcode validation: PASSED\n")

# Step 4.4: Create a new Giotto object with ONT gene expression and spatial locations
# ------------------------------------------------------------------------------------
SpatialTrans_DMSO_ONTgene <- createGiottoObject(
  expression = SpatialTrans_DMSO_ONTgene, 
  spatial_locs = spatlocs_filtered,
  instructions = instrs            # Use the instructions we created
)
```

# Step 4.5: Transfer Image from Original Giotto Object

```{r step4_5_transfer_image}
...
# Step 4.6: Transfer Cell Metadata and Spatial Information

```{r step4_6_transfer_metadata}
# Transfer cell metadata from the original object
SpatialTrans_DMSO_ONTgene@cell_metadata$cell$rna@metaDT <- 
  SpatialTrans_DMSO@cell_metadata$cell$rna@metaDT

# Transfer spatial information from the original object
...
# Verify the updated Giotto object
...
Use the function addStatistics() to count the number of features per spot. The statistics information will be stored in the metadata table under the new column "nr_feats".

```{r step7_qc_statistics}
# Add statistics
...
## Step 7a: Compute Mitochondrial and Ribosomal QC Metrics

```{r step7a_mito_ribo_qc}
# Get raw expression matrix for QC calculations
expr_matrix <- getExpression(SpatialTrans_DMSO_ONTgene_statistics, 
                              values = "raw", 
                              output = "matrix")
...
cell_meta <- getCellMetadata(SpatialTrans_DMSO_ONTgene_statistics, output = "data.table")
...
SpatialTrans_DMSO_ONTgene_statistics <- addCellMetadata(
...
## Step 7b: Visualize %Mito and %Ribo Spatially

```{r step7b_mito_ribo_plots}
...
par(mfrow = c(1, 2))
hist(pct_mito, breaks = 50, main = "Distribution of %Mito", xlab = "%Mitochondrial", col = "salmon")
abline(v = median(pct_mito), col = "red", lwd = 2, lty = 2)
hist(pct_ribo, breaks = 50, main = "Distribution of %Ribo", xlab = "%Ribosomal", col = "lightblue")
abline(v = median(pct_ribo), col = "blue", lwd = 2, lty = 2)
par(mfrow = c(1, 1))
```

## Step 7c: Standard QC Visualization

```{r step7c_qc_nr_feats}
...
# Step 7.1: Distribution of Features per Spot

```{r step7_1_dist_cells}
...
feat_det_in_min_spots <- 30  # Reducing this leads to differential expression of few spots
...
# Add statistics for visualization (post-filter)
SpatialTrans_DMSO_ONTgene <- addStatistics(
  gobject = SpatialTrans_DMSO_ONTgene,
  expression_values = "raw"
)
...
post_filter_meta <- getCellMetadata(SpatialTrans_DMSO_ONTgene, output = "data.table")
cat("\n=== Post-filter QC Summary ===\n")
cat("Remaining spots:", nrow(post_filter_meta), "\n")
...
SpatPlot2_QC_afterfiltering_gg <- SpatPlot2_QC_afterfiltering + 
  ...
  annotate(
    "label", 
    x = Inf, y = -Inf, 
    label = "Spots Filtering Criteria",
    hjust = 1.05, vjust = -4.8, size = 4, color = "black", fontface = "bold",
    fill = "white", alpha = 0.9, label.padding = unit(0.3, "lines")
  ) +
  # Add the filtering criteria details below the title
  annotate(
    "label", 
    x = Inf, y = -Inf, 
    label = paste0(
      ...
      "• Min detected features per spot: ", min_det_feats_per_spot
    ),
    hjust = 1.05, vjust = -0.1, size = 3.5, color = "black", 
    fill = "white", alpha = 0.85, label.padding = unit(0.4, "lines")
  )
...
## Step 8a: Post-filter %Mito and %Ribo QC (Sensitivity Analysis)

```{r step8a_postfilter_mito_ribo}
# Re-compute %Mito and %Ribo after filtering for sensitivity analysis
expr_matrix_post <- getExpression(SpatialTrans_DMSO_ONTgene, 
                                   values = "raw", 
                                   output = "matrix")
...
# Add to metadata
post_meta <- getCellMetadata(SpatialTrans_DMSO_ONTgene, output = "data.table")
...
SpatialTrans_DMSO_ONTgene <- addCellMetadata(
  gobject = SpatialTrans_DMSO_ONTgene,
...
# Step 10: Highly Variable Features (HVF)
...
## 11.1 Run PCA Using HVFs

```{r step11_1_pca}
# Set seed for reproducibility
set.seed(123)

# Run PCA using highly variable features
...
## 11.2 Run Custom PCA with Selected Features

```{r step11_2_custom_pca}
...
# Step 13: Create Nearest Neighbor Networks
...
# Step 14: Leiden Clustering

## 14.1 Calculate Leiden Clusters and Visualize in PCA Space

```{r step14_1_leiden_pca}
# Set seed for reproducibility of clustering results
set.seed(123)

# Calculate Leiden clustering with a resolution of 1.2
...
PCAplot_LeidenClusters <- Giotto::plotPCA(
  gobject = SpatialTrans_DMSO_ONTgene,
...
## 14.2 Visualize Leiden Clusters Spatially

```{r step14_2_leiden_spatial}
...
# Get distinct colors for 5 clusters
raw_colors <- Giotto::getDistinctColors(5)
# Reorder colors: 1->1, 2->5, 3->2, 4->3, 5->4 based on user request
cluster_colors <- raw_colors[c(1, 5, 2, 3, 4)]
names(cluster_colors) <- 1:5
...
Tumouroid_3 <- spatInSituPlotPoints(
  ...
  ggplot2::guides(fill = ggplot2::guide_legend(override.aes = list(shape = 21, size = 8, stroke = 0.5))) +
  ggplot2::labs(
    x = "x coordinates",
    y = "y coordinates",
    fill = "Leiden Clusters"  # Add legend title
...
## Combine Plots for A in Figure 1

```{r step15_combine_tumouroids}
library(patchwork)
...
# Interactive Region Selector for Spatial Transcriptomics (Optional)

This interactive Shiny app allows you to select regions from spatial plots. Run only if needed.

```{r interactive_region_selector, eval=FALSE}
# Source the external Shiny app script for interactive region selection
# Requires: SpatialTrans_DMSO_ONTgene object to be loaded
source("./scripts/interactive_region_selector.R")
```
...
# Filter for upregulated genes only (logFC > 0) for visualization
scran_markers_up <- scran_markers[logFC > 0]

# Get top 8 upregulated genes per cluster for heatmap/visualization
...
violinPlot_scranMarker <- violinPlot(SpatialTrans_DMSO_ONTgene, 
           ...
           save_param = modifyList(base_save_parameters, list(base_width = 15, base_height = 30)),    # Override base width and height 
...
```{r step16_spatial_dist_scran}
Spatial_Dist_scranMarkers <- spatFeatPlot2D(SpatialTrans_DMSO_ONTgene, 
              expression_values = "scaled",
              ...
              point_size = 2.5,
              point_alpha = 0.7,
              save_param = modifyList(base_save_parameters, list(base_width = 20, base_height = 20)),
              return_plot = TRUE,
              default_save_name = "Spatial_Dist_scranMarkers"
              )
print(Spatial_Dist_scranMarkers)
          
```

```{r step16_spatial_top_scran}
...
# Get top 250 significant UPREGULATED genes per cluster (logFC > 0)
...
# Step 17: Enrichment and Deconvolution
...
# Step 18: Spatial Expression Patterns

## Patial variable genes

Create a spatial network

```{r step18_spatial_network}
